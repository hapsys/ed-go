...
public class DBStationsAccess extends Access {
	...
	public DBStationsBean getByMarketId(java.math.BigInteger paramMarketId)  throws SQLException, IllegalArgumentException, IllegalAccessException, InstantiationException {
		setNames();
		DBStationsBean ret = null;
		SqlInjectorInterface injector = new EmptySqlInjector();
		
		String sql = "SELECT t.* "+injector.getRecordQuery()+" FROM " + tablename + " as t "+injector.getFromQuery()+" WHERE 1=1  AND  market_id= ?  "+injector.getWhereQuery()+" ";
		if (injector.getOrderQuery().length() != 0) {
			sql += injector.getOrderQuery();
		} else { 
		...
		List<Map<String, Object>> result = getConnection().fetchRows(tablename + ".getByMarketId", sql ,  paramMarketId);
		if (result != null) {
