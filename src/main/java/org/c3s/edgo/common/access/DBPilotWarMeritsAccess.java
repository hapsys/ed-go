...
public class DBPilotWarMeritsAccess extends Access {
	...
	public List<DBPilotWarMeritsBean> getListForPilots(org.c3s.edgo.common.intruders.InInjector paramIntruder) throws SQLException, IllegalArgumentException, IllegalAccessException, InstantiationException {
		...
		if (query == null) {
			...
			String limit = injector.getLimitQuery();
			query = " 				SELECT t.*, s.name as system_name, SUM(t.quantity) as quantity_summ 				FROM pilot_war_merits t, systems s 				WHERE 1=1 				" + where + " 				AND (t.is_confirmed = 1 OR t.is_confirmed = 0) 				AND t.system_id = s.system_id 				GROUP BY t.start_week, t.system_id, t.is_confirmed  				ORDER BY start_week DESC 			";
		}
