...
public class Location extends AbstractJournalEvent<LocationBean> {
	...
	protected void processBean(LocationBean bean) {
		...
		try {
			...
			if (pilot != null) {
				LocationDAO.insertLocation(pilot.getPilotId(), bean.getTimestamp(), bean.getStarSystem(), bean.getStarPos(), bean.getStationName(), bean.getStationType(), null, bean.getBody(), bean.getBodyType());
